...
export const AdminApprovalsModule: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'pending' | 'revision' | 'approved' | 'rejected' | 'all'>('pending');
    ...
    try {
      ...
      try {
        ...
        orderList.forEach((ord: any) => {
          ...
          const isRevision = ord.status === 'revision_requested';
          ...
          if (isPendingReview || isApproved || isRejected || isRevision) {
            ...
            items.push({
              ...
              status: isRevision ? 'revision' : isPendingReview ? 'pending' : isApproved ? 'approved' : 'rejected',
              ...
              currentVersion: ord.current_version || 1,
              revisionRequests: ord.revision_requests || [],
              isRevisionRequested: isRevision,
            } as any);
          }
        });
      } catch (e) {
    ...
    <div className="space-y-6">
        ...
        {(['pending', 'revision', 'approved', 'rejected', 'all'] as const).map((tab) => {
          const count =
            tab === 'pending'
              ...
              : tab === 'revision'
              ? approvals.filter((a) => (a as any).status === 'revision').length
              : tab === 'approved'
          ...
          return (
            ...
            >
              <span>{label}</span>
              {count > 0 && (
                <span className={`px-1.5 py-0.5 rounded-full font-mono font-bold text-[10px] ${
                  activeFilter === tab
                    ? 'bg-[#C9A227] text-[#0D1B4C]'
                    : tab === 'revision'
              ...
              <div>
                ...
                >
                  ...
                  <div className="absolute top-3 right-3 pointer-events-none flex flex-col items-end gap-1">
                    {(item as any).currentVersion && (item as any).currentVersion > 1 && (
                      <span className="px-2 py-0.5 rounded bg-black/80 backdrop-blur text-[#F5E7A3] font-mono font-bold text-[10px] border border-[#D4AF37]/50 shadow-md">
                        Deliverable v{(item as any).currentVersion}
                      </span>
                    )}
                    {item.status === 'pending' && (
                    ...
                    {(item as any).status === 'revision' && (
                      <span className="px-2.5 py-1 rounded-full bg-amber-600 text-white font-bold text-[10px] uppercase shadow-md flex items-center gap-1">
                ...
                <div className="p-5 space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {(item as any).currentVersion && (item as any).currentVersion > 1 && (
                      <span className="px-2 py-0.5 rounded bg-blue-50 text-blue-900 border border-blue-200 text-[10px] font-mono font-bold shrink-0">
                        v{(item as any).currentVersion}
                      </span>
                    )}
                  </div>

                  {(item as any).revisionRequests && (item as any).revisionRequests.length > 0 && (
                    <div className="p-2.5 rounded-xl bg-amber-50 border border-amber-200 text-xs space-y-1">
                      <div className="flex items-center justify-between text-[10px] font-bold text-amber-900">
                        <span className="flex items-center gap-1">
                          ...
                          Client Revision Feedback ({(item as any).revisionRequests.length})
                        </span>
                        <span className="font-mono text-[9px] uppercase px-1.5 py-0.5 rounded bg-amber-200 text-amber-900">
                          {((item as any).revisionRequests[0] || {}).status || 'Requested'}
                        </span>
                      </div>
                      <p className="text-amber-950 text-[11px] italic line-clamp-2">
                        "{((item as any).revisionRequests[0] || {}).comment}"
                      </p>
                    </div>
                  )}
      ...
      {inspectingItem && (
        ...
        >
          ...
          {/* Client Revision Request Instructions (if any feedback is logged) */}
          {(inspectingItem as any).revisionRequests && (inspectingItem as any).revisionRequests.length > 0 && (
            <div className="w-full bg-amber-950/40 border border-amber-500/40 rounded-2xl p-4 space-y-2.5 shadow-xl">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono font-bold text-amber-300 uppercase tracking-wider flex items-center gap-1.5">
                  <MessageSquare className="w-4 h-4 text-amber-400" />
                  Client Revision Instructions ({(inspectingItem as any).revisionRequests.length} Iterations Logged)
                </span>
                <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-amber-900/60 text-amber-200 border border-amber-500/30">
                  Target Version: v{(inspectingItem as any).currentVersion || 1}
                </span>
              </div>
              <div className="space-y-2 max-h-44 overflow-y-auto pr-1">
                {(inspectingItem as any).revisionRequests.map((rev: any, idx: number) => (
                  <div key={rev.id || idx} className="p-3 rounded-xl bg-black/40 border border-amber-500/20 text-xs space-y-2">
                    <div className="flex items-center justify-between text-[11px]">
                      <span className="font-semibold text-white font-serif">
                        Revision #{rev.revision_number} &bull; Against Deliverable v{rev.deliverable_version}
                      </span>
                      <span className="text-[10px] font-mono text-amber-300 uppercase px-2 py-0.5 rounded bg-amber-950 border border-amber-500/30">
                        {rev.status}
                      </span>
                    </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3 pt-1">
                        ...
                        {rev.reference_image && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="flex items-center gap-1 text-xs text-[#F5E7A3] hover:underline"
                          >
                            <ImageIcon className="w-3.5 h-3.5" />
                            <span>View Customer Markup Reference</span>
                          </a>
                        )}
                      </div>
                    )}
          ...
          <div className="border-t border-white/10 pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-white">
            ...
            <div className="flex items-center gap-3">
              {inspectingItem.status === 'pending' || (inspectingItem as any).status === 'revision' ? (
                <>
                  <button
                    onClick={() => {
